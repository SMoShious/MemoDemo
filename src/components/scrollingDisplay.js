...
var display0 = createDisplay(120, 30, 40, "Black", "White", "#312930", 10, 2, 1);
setText("HELLO", display0);

var display1 = createDisplay(10, 200, 85, "Black", "Orange", 51, 7, 2, 1);
setText(" I love to code on CodeGuppy.com", display1);

var display2 = createDisplay(10, 350, 128, "Olive", "Lime", "DarkGreen", 5, 1, 2);
setText("JavaScript is a cool programming language...", display2);

var displayClock = createDisplay(10, 560, 64, "#E0E0E0", "Black", "White", 2, 1, 2);

var displayInfo = createDisplay(290, 570, 500, "", "Blue", "", 1, 0, 2);
setText(" LED scrolling effects. Feel free to customize this program...", displayInfo);
...
function loop()
{
    ...
    clearDisplay(displayClock);
    printText(getTime(), displayClock, 0);
    display(displayClock);
    
    scroll(displayInfo, 1);
    display(displayInfo);
}

function createDisplay(x, y, noCols, background, ledOn, ledOff, dotSize, dotSpace, dotStyle)
{
    function createBuffer(noCols)
    {
        var mat = new Array(8);
    ...
    var display = {
      ...
      dotSpace : dotSpace,
      dotStyle : dotStyle,
      buff : createBuffer(noCols)
    }

    return display;
}

function setText(txt, display)
{
    display.text = txt;
    display.textOffset = 0;
    
    printText(display.text, display, 0);
}

function scroll(display, speed)
{
    ...
    printText(display.text, display, display.textOffset);
    
    if (frameCount % speed == 0)
        display.textOffset--;
    
    if (display.textOffset < -display.text.length * 8)
    {
        display.textOffset = display.noCols;
    }
}
...
function display(display)
{
    ...
    if (display.background)
    {
        ...
        rect(display.x, display.y, displayWidth(display), displayHeight(display));
    }
...
function displayDot(row, col, state, display)
{
    if (state != 1 && !display.ledOff)
        return;

    var xd = display.x + col * (display.dotSize + display.dotSpace);
    var yd = display.y + row * (display.dotSize + display.dotSpace);

    fill( state == 1 ? display.ledOn : display.ledOff );
...
{
    var h = 8 * display.dotSize + 7 * display.dotSpace;
    return h;
}
...
function printText(text, display, displaycol)
{
    for(var i = 0; i < text.length; i++)
    {
        ...
        printChar(chr, display, col);
    }
}


function printChar(chr, display, displaycol)
{
    ...
    for(var row = 0; row < bmp.length; row++)
    {
        ...
        {
            if (t[col] == 1)
                printDot(display, row, displaycol + col);
        }
    }
}

function printDot(display, dotrow, dotcol)
{
    var buff = display.buff;
